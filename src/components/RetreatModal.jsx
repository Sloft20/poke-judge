// src/components/RetreatModal.jsx
        ...
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-200">
            <div className="w-full max-w-md bg-slate-900 border border-orange-900/50 rounded-3xl shadow-2xl overflow-hidden flex flex-col relative">
                ...
                <div className="bg-slate-950 p-6 border-b border-slate-800 flex justify-between items-start relative overflow-hidden">
                    ...
                    <div>
                        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3">
                            <span className="text-orange-500 bg-orange-900/20 p-2 rounded-lg">
                ...
                <div className="p-6 bg-slate-900 flex flex-col gap-6">
                    <div className="bg-orange-950/30 border border-orange-500/20 p-4 rounded-xl flex items-start gap-3">
                                ...
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${isSelected 
                                            ...
                                            : 'border-slate-800 bg-slate-800/50 hover:border-slate-600 hover:bg-slate-700'}
                                    `}
                                >
                                    <div className={`${isSelected ? 'text-white' : 'text-slate-500'} transition-colors transform group-hover:scale-110 duration-200`}>
                                        <Icon size={24} />
                                    </div>
                                    {isSelected && (
                                        <div className="absolute -top-2 -right-2 bg-orange-500 text-white rounded-full p-0.5 shadow-sm border-2 border-slate-900">
                ...
                <div className="p-4 bg-slate-950 border-t border-slate-800 flex gap-3">
                    <button onClick={onCancel} className="flex-1 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-800 hover:text-white transition-colors uppercase text-xs tracking-wider border border-transparent hover:border-slate-700">
                        Cancelar
                    </button>
                    <button onClick={() => onConfirm(selectedIndices)} disabled={!isReady} className={`flex-1 py-3 rounded-xl font-black uppercase text-xs tracking-widest transition-all shadow-lg flex items-center justify-center gap-2 ${isReady ? 'bg-gradient-to-r from-orange-600 to-red-600 text-white hover:shadow-orange-500/25 hover:scale-[1.02]' : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}>
