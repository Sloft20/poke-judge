...
const PokemonCard = ({ card, location = 'bench', onClick, isActive = false, getMaxHP }) => {
    ...
    // --- 1. LÓGICA DE TEXTO (A CORREÇÃO PRINCIPAL) ---
    // Só definimos texto se for uma das 3 condições de rotação
    let statusText = null;
    if (condition === 'ASLEEP') statusText = 'Dormindo';
    else if (condition === 'PARALYZED') statusText = 'Paralisado';
    else if (condition === 'CONFUSED') statusText = 'Confuso';
    // Se for qualquer outra coisa (ex: 'NONE', 'BURNED'), statusText continua null e não desenha nada.

    // --- 2. LÓGICA DE ROTAÇÃO ---
    // Forçamos rotate-0 se não for uma condição de rotação
    ...
    if (isActive) {
        ringClass = "ring-4 ring-yellow-400 shadow-xl z-10 scale-105"; 
    }
    ...
    return (
        ...
        >
            ...
            {/* ELEMENTOS FLUTUANTES (Não giram) */}
            ...
            {/* --- ETIQUETA DE STATUS (CORRIGIDA) --- */}
            {/* Só renderiza se statusText tiver conteúdo real */}
            {statusText && (
                <div className="absolute bottom-4 left-0 right-0 text-center z-30 pointer-events-none animate-in fade-in slide-in-from-bottom-2">
                    <span className="bg-red-600 text-white text-[9px] font-bold px-3 py-1 rounded-full shadow-lg border border-white/20 uppercase tracking-widest">
                        {statusText}
                    </span>
                </div>
            )}
